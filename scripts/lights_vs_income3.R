...
income_lights_scaled <- income_lights %>% 
  mutate_each_(list(~scale(.) %>% as.vector), 
               vars=c("radiance_mean","radiance_median","radiance_max",
                      "radiance_min","radiance_std","income"))
...
# Level variables
income_lights <- income_lights %>% data.matrix()
lambdas <- 10^seq(3, -2, by = -.1)
cv_fit <- cv.glmnet(income_lights[, 1:5], income_lights[, 6], alpha = 0, 
                    lambda = lambdas, nfolds = 10)
opt_lambda <- cv_fit$lambda.min
fit <- cv_fit$glmnet.fit
y_predicted <- predict(fit, s = opt_lambda, newx = income_lights[, 1:5])
# Sum of Squares Total and Error
sst <- sum((income_lights[, 6] - mean(income_lights[, 6]))^2)
sse <- sum((y_predicted - income_lights[, 6])^2)
# R squared
rsq <- 1 - sse / sst
rsq
...
# Scaled variables
income_lights_scaled <- income_lights_scaled %>% data.matrix()
lambdas <- 10^seq(3, -2, by = -.1)
cv_fit <- cv.glmnet(income_lights_scaled[, 1:5], income_lights_scaled[, 6], alpha = 0, 
                    lambda = lambdas, nfolds = 10)
opt_lambda <- cv_fit$lambda.min
fit <- cv_fit$glmnet.fit
y_predicted <- predict(fit, s = opt_lambda, newx = income_lights_scaled[, 1:5])
# Sum of Squares Total and Error
sst <- sum((income_lights_scaled[, 6] - mean(income_lights_scaled[, 6]))^2)
sse <- sum((y_predicted - income_lights_scaled[, 6])^2)
# R squared
rsq <- 1 - sse / sst
rsq
