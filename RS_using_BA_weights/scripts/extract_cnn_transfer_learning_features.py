...
json_file = open('../BA/input/model/model.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
model = model_from_json(loaded_model_json)
model.save_weights('../BA/input/model/weights.hdf5')
...
income_city_files = pd.read_csv('../RS/input/model/income_city_file.txt')
...
for city in income_city_files['city_code'].unique():
    ...
    for i in range(df_filter.shape[0]):
        img = image.load_img('../RS/' + df_filter.iloc[i, [2]][0])
