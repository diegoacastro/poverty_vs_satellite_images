...
cv_fit <- train(images_water[1:4077], images_water$water_index, 
                method = "glmnet", 
                preProcess = c("pca"),
...
cv_fit <- train(images_water[1:4077], images_water$water_index_log, 
                method = "glmnet", 
                preProcess = c("pca"),
                trControl = trainControl(method="cv", 
                                         number=10, 
...
for(i in c("no sampling", "down", "up", "smote")){
  ...
  cv_fit <- train(images_water[, 1:4034], images_water$label_class, 
                  method = "glmnet", 
                  family="binomial",
                  preProcess = c("pca"),
                  trControl = ctrl,
...
#   
#   
#   cv_fit <- train(as.data.frame(images_water[, 1:4034]), images_water$label_class, 
#                   method = "glmnet", 
#                   family="binomial",
#                   preProcess = c("pca", "center", "scale"),
#                   trControl = ctrl,
