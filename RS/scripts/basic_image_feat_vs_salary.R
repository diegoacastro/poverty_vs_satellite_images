...
images_salary <- features_basic %>% 
  ...
  mutate(average_wage_log = log(average_wage),
         label_class = ifelse(average_wage <= 1562.858, # Median of average monthly wage
                              "low salary", 
                              "high salary")) %>% 
  select(-city_code)
...
for(i in c("no sampling", "down", "up", "smote")){
  ...
  cv_fit <- train(as.data.frame(images_salary[, 1:15]), images_salary$label_class, 
                  method = "glmnet", 
                  family="binomial",
                  trControl = ctrl,
                  metric = "Accuracy",
...
for(i in c("no sampling", "down", "up", "smote")){
  ...
  stratified_kfold <- createFolds(y = factor(images_salary_feat_scaled$label_class), 
                                  k = 10, 
                                  returnTrain = TRUE)
  ...
  cv_fit <- train(as.data.frame(images_salary_feat_scaled[, 1:15]), images_salary_feat_scaled$label_class, 
                  method = "glmnet", 
                  family="binomial",
                  trControl = ctrl,
                  metric = "Accuracy",
