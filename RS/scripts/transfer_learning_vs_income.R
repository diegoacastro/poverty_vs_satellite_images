...
images_income <- images_income %>% 
  ...
  mutate(label_class = ifelse(income <= 537.16, # Median of average income
                              "low income", 
                              "high income"))
...
for(i in c("no sampling", "down", "up", "smote")){
  ...
  cv_fit <- train(images_income[, 1:4096], images_income$label_class, 
                  method = "glmnet", 
                  family="binomial",
                  preProcess = c("pca"),
                  trControl = ctrl,
...
#   
#   
#   cv_fit <- train(as.data.frame(images_income[, 1:4096]), images_income$label_class, 
#                   method = "glmnet", 
#                   family="binomial",
#                   preProcess = c("pca", "center", "scale"),
#                   trControl = ctrl,
