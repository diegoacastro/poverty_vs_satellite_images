...
### Date: 25/07/2019
### Objective: evaluate transfer learning model on average salary
...
cv_fit <- train(images_salary[1:4096], images_salary$average_wage, 
                method = "glmnet", 
                preProcess = c("pca"),
...
cv_fit <- train(images_salary[1:4096], images_salary$average_wage_log, 
                method = "glmnet", 
                preProcess = c("pca"),
                trControl = trainControl(method="cv", 
                                         number=10, 
...
for(i in c("no sampling", "down", "up", "smote")){
  ...
  cv_fit <- train(images_salary[, 1:4096], images_salary$label_class, 
                  method = "glmnet", 
                  family="binomial",
                  preProcess = c("pca"),
                  trControl = ctrl,
...
#   
#   
#   cv_fit <- train(as.data.frame(images_salary[, 1:4096]), images_salary$label_class, 
#                   method = "glmnet", 
#                   family="binomial",
#                   preProcess = c("pca", "center", "scale"),
#                   trControl = ctrl,
